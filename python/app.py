...
for row in rows:
	try:
 		...
 		INSERT INTO [dbo].[Block] (number, hash, parent_hash, nonce, sha3_uncles, logs_bloom , transactions_root, state_root , receipts_root, miner, difficulty, total_difficulty, size, extra_data, gas_limit, gas_used, timestamp, transaction_count, base_fee_per_gas) 
 		VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
...
for row in rows:
	try:
		count = cursor.execute("""
		INSERT INTO [dbo].[Block Transactions] (hash, nonce, block_hash, block_number, transaction_index, from_address , to_address, value , gas, gas_price, input, block_timestamp, max_fee_per_gas, max_priority_fee_per_gas, transaction_type, sensor_id) 
		VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
